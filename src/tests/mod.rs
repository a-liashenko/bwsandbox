...
#[test]
fn test_slirp4netns() {
    // Test that sandboxed process can't access parent network

    // Local server, should be isolated from sandbox
    let socket = std::net::TcpListener::bind("127.0.0.1:0").unwrap();
    ...
    let local_addr = format!("telnet://127.0.0.1:{port}");
    let args = vec![
        "-f",
        "./profiles/with-slirp4netns.toml",
        "--",
    ...
    let output = cargo_spawn_out(args).unwrap();
    dbg!(output.stdout_str());
    dbg!(output.stderr_str());
}
